        ...
        <Box>
          {edit ? (
            <>
              ...
              <FormControlLabel
                sx={{ padding: "0 10px", background: "white", color: "black", marginLeft: "0px" }}
                control={<Checkbox checked={isPublic} onChange={() => setIsPublic((old) => !old)} />}
                label="Public"
              />
            </>
          ) : (
        ...
        {isPlaylistEditable && (
          <Grid alignSelf={"end"} item xs={2} sm container>
            <Grid item xs container display={"block"} alignSelf={"flex-end"} direction="column" spacing={2}>
              ...
              {edit && (
                <Button onClick={() => setEdit(false)} sx={{ float: "right", width: "auto", maxWidth: "300px", textTransform: "none" }} variant="contained" color="warning">
                  Cancel
                </Button>
              )}
            </Grid>
